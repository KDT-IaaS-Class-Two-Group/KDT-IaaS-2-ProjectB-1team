...
#     # 'your_desired_folder'를 원하는 폴더명으로 변경해줘
#     os.makedirs(desired_folder, exist_ok=True)  # 폴더가 없으면 생성

#     # 이미지 저장 경로 설장
#     output_image_path = os.path.join(desired_folder, f"success_{os.path.basename(image_path)}")
...
def process_image(image_path):
    ...
    if status == "success":
        ...
        existing_data[f"fail_{os.path.basename(image_path)}"] = {}
